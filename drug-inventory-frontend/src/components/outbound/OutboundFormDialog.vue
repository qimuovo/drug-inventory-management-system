<template>
  <el-dialog :model-value="modelValue" title="新增出库" width="900px" destroy-on-close @close="handleClose">
    <el-form ref="formRef" :model="formData" :rules="rules" label-width="90px">
      <el-row :gutter="12">
        <el-col :span="8">
          <el-form-item label="出库单号" prop="outboundNo">
            <el-input v-model="formData.outboundNo" placeholder="请输入出库单号" maxlength="64" />
          </el-form-item>
        </el-col>
        <el-col :span="8">
    ...
    <template #footer>
      ...
      <el-button type="primary" :loading="loading" @click="handleSubmit">提交出库</el-button>
    </template>
  </el-dialog>
</template>
...
type DrugOption = { id: number; label: string }

const props = withDefaults(defineProps<{ modelValue: boolean; loading?: boolean }>(), { loading: false })
const emit = defineEmits<{
  (e: 'update:modelValue', value: boolean): void
  (e: 'submit', value: API.OutboundAddRequest): void
}>()

const formRef = ref<FormInstance>()
let uidSeed = 0
const formData = reactive({ outboundNo: '', outboundDate: '', remark: '' })
const rules: FormRules = { outboundNo: [{ required: true, message: '请输入出库单号', trigger: 'blur' }] }
...
const createEmptyItem = (): LocalItem => ({ uid: ++uidSeed, drugId: undefined, batchNo: '', quantity: 1, price: 0 })
const resetState = () => {
  formData.outboundNo = ''
...
const loadDrugOptions = async (reset = false) => {
  ...
  try {
    ...
    const currentOptions = (data.records ?? []).map((item) => ({
      id: Number(item.id),
...
const addItem = () => itemList.value.push(createEmptyItem())
const removeItem = (index: number) => {
  if (itemList.value.length === 1) return ElMessage.warning('至少保留一条明细')
...
const handleDrugVisibleChange = (visible: boolean) => {
  if (visible && !drugOptions.value.length) loadDrugOptions(true)
}
const handleDrugPopupScroll = (event: Event) => {
  const target = event.target as HTMLElement
  if (!target || drugLoading.value || !hasMoreDrug.value) return
  if (target.scrollTop + target.clientHeight >= target.scrollHeight - 32) loadDrugOptions()
}

const handleSubmit = async () => {
  await formRef.value?.validate()
  const payloadItemList: API.OutboundAddItemRequest[] = []
  for (const item of itemList.value) {
    if (!item.drugId || item.drugId <= 0) return ElMessage.warning('请选择药品')
    if (!item.batchNo.trim()) return ElMessage.warning('请填写批号')
    if (!item.quantity || item.quantity <= 0) return ElMessage.warning('出库数量必须大于 0')
    if (item.price < 0) return ElMessage.warning('出库单价不能小于 0')
    payloadItemList.push({ drugId: item.drugId, batchNo: item.batchNo.trim(), quantity: item.quantity, price: item.price })
  }
  emit('submit', {
    outboundNo: formData.outboundNo.trim(),
    outboundDate: formData.outboundDate || undefined,
...
.summary { margin-top: 8px; text-align: right; color: var(--el-text-color-secondary); }
</style>
